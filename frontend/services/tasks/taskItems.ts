...
export const createTaskItem = async (
  ...
  const res = await api.post<Task>(`/api/task-lists/${data.listId}/items`, data);
  return res.data;
};

export const toggleTaskItem = async (listId: string, itemId: string, completed: boolean): Promise<Task> => {
  const res = await api.patch<Task>(`/api/task-lists/${listId}/items/${itemId}`, { completed });
  return res.data;
};
